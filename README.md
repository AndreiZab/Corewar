...
### Ассемблер
Это программа, которая будет транслировать написанный на языке ассемблера код чемпиона в байт-код(машинный код), который будет интерпретироваться виртуальной машиной напрямую.

Этапы:
* Лексический анализ ассемблер кода и получение токенов(лексем) из него
* Валидация лексем 
* Трансляция полученных команд в байт код
